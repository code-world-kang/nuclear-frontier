...
- **翻译透明**：原生中文、题目、摘要/介绍、部分译文和原文缺失分开统计。无服务时只建立待译队列。GitHub Models 已退休，不再使用其接口和权限，也不把其他模型的结果标成 Codex。可在仓库配置 `TRANSLATION_API_URL`、`TRANSLATION_MODEL` 和 Secret `TRANSLATION_API_KEY` 接入服务；本次未配置收费服务或生成整库译文。
